...
controller.getUsuario = async(req,res) => {
    ...
    if(respuesta.rows.length > 0){
            return res.status(200).json({codigo:200,usuario:respuesta.rows[0]});
    } else {
            return res.status(500).json({codigo:500,error:'Error al solicitar datos de usuario'})
    }
}
...
controller.postUsuario = async(req,res) => {
    try{
        ...
        if(respuesta.rows.length > 0){
                return res.status(201).json({codigo:201,id:respuesta.rows[0].id});
        } else {
                return res.status(500).json({codigo:500,error:'Error al crear usuario'})
        }
    } catch (error){
...
controller.putUsuario = async(req,res) => {
    try{
        ...
        const respuesta = await pool.query(sentenciaUpdate)
            ...
            return res.status(204).json({codigo:204});
    } catch (error){
        console.log(error)
            return res.status(error[0]).json({codigo:error[0],error: error[1]})
    }
